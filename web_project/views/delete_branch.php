...
<div   id="delete_r" class="container">
    <div>
        <form method="POST" action="/SE/delete_branch">
            <div class="form-group">
                <div class="form-inline">
                    <label class="laform">ค้นหาโดย:</label>
                    <input type="radio" name="search" value="code" checked="checked">รหัสสาขา
                    <input type="radio" name="search" value="name">ชื่อสาขา
    ...
    { 
        if($_POST["chkDel"][$i] != "")
        { 
            $strSQL = 'DELETE FROM branch where branch_id like "'.$_POST["chkDel"][$i].'"' ;
    ...
    ?>
        ...
        $query='SELECT branch_id,code as "รหัสสาขา",name as "ชื่อ - นามสกุล" FROM branch where '.$tb.' like "'.$kw.'%" ';       
        ...
        <table class="table table-bordered" id="alert"  >
            ...
            <tbody>
                ...
                <tr class="<?php echo $colorCh; ?>">
                    <td align="center" ><input type="checkbox" name="chkDel[]" id="chkDel<?php echo $j;?>" value="<?php echo $row["branch_id"];?>"></td>
                    <?php for ($i = 0; $i < mysql_num_fields($result); $i++): ?>
                        <?php if($i==0): ?>
